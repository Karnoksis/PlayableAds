...
namespace KWS
{
    public static partial class KWS_CoreUtils
    {
        ...
        {
            var cs = Resources.Load(shaderName);
            if (cs == null)
            {
        ...
        public static Bounds BoundsLocalToWorld(Bounds localBounds, Transform transform, Vector3 scale)
        {
            var center = transform.TransformPoint(Vector3.Scale(localBounds.center, scale));

            // transform the local extents' axes
            var extents = Vector3.Scale(localBounds.extents, scale);
