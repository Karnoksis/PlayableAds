...
namespace KWS
{
    public class ScreenSpaceReflectionPassCore : WaterPassCore
    {
        ...
        void InitializeMaterials(WaterSystem waterInstance)
        {
            if (_cs == null)
            {
                _cs = KWS_CoreUtils.LoadComputeShader($"PlatformSpecific/KWS_ScreenSpaceReflectionCompute");
                if (_cs == null) return;
        ...
        public void Execute(Camera cam, CommandBuffer cmd, RenderTargetIdentifier depthBuffer, RenderTargetIdentifier colorBuffer)
        {
            ...
            cmd.SetComputeTextureParam(_cs, _kernelRenderColorFromHash, KWS_ShaderConstants.SSR_ID._CameraOpaqueTexture, colorBuffer);
