...
namespace KWS
{
    public class FFT_GPU
    {
        ...
        void InitializeResources()
        {
            ...
            if (_cmd == null) _cmd = new CommandBuffer() { name = "Water.RenderingWavesFFT" };
        ...
        {
            _cmd.SetComputeFloatParam(spectrumShader, "time", timeOffset);
            _cmd.SetComputeTextureParam(spectrumShader, kernelSpectrumUpdate, "init0", spectrumInitRenderTexture);
            _cmd.SetComputeTextureParam(spectrumShader, kernelSpectrumUpdate, "resultHeight", spectrumHeight);
            _cmd.SetComputeTextureParam(spectrumShader, kernelSpectrumUpdate, "resultDisplaceX", spectrumDisplaceX);
            _cmd.SetComputeTextureParam(spectrumShader, kernelSpectrumUpdate, "resultDisplaceZ", spectrumDisplaceZ);

            _cmd.DispatchCompute(spectrumShader, kernelSpectrumUpdate, _currentSize / 8, _currentSize / 8, 1);
        }
        ...
        void DispatchFFT(float windSpeed)
        {
            ...
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset, "inputH", spectrumHeight);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset, "inputX", spectrumDisplaceX);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset, "inputZ", spectrumDisplaceZ);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset, "inputButterfly", texButterfly);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset, "output1", fftTemp1);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset, "output2", fftTemp2);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset, "output3", fftTemp3);
            _cmd.DispatchCompute(shaderFFT, kernelOffset, 1, _currentSize, 1);

            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset + 1, "inputH", fftTemp1);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset + 1, "inputX", fftTemp2);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset + 1, "inputZ", fftTemp3);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset + 1, "inputButterfly", texButterfly);
            _cmd.SetComputeTextureParam(shaderFFT, kernelOffset + 1, "output", DisplaceTexture);
            _cmd.DispatchCompute(shaderFFT, kernelOffset + 1, _currentSize, 1, 1);


            _cmd.SetGlobalFloat("KW_FFTDomainSize", KWS_Settings.Water.FftDomainSize[_lodIdx]);
            _cmd.SetGlobalTexture("_DispTex", DisplaceTexture);
            var sizeLog = Mathf.RoundToInt(Mathf.Log(_currentSize, 2)) - 4;
            _cmd.SetGlobalFloat("_SizeLog", sizeLog);
            _cmd.SetGlobalFloat("_WindSpeed", windSpeed);

            _cmd.Blit(null, NormalTexture, normalComputeMaterial, 0);
